...
if 'predicted_aligned_error' in json_dict.keys():
    ...
    if prot_len1 != prot_len2:
        print (f'error, matrix is not a square with shape ({prot_len1}, {prot_len2})')
    else:
        length = prot_len1
    ...
    for res2 in RESIDS:
        if res1 < res2:
            ...
            if check_dist and check_error and check_hhpro and check_daliz:
                ...
                print (f'error\t{prefix}\t{str(res1)}\t{str(res2)}')

chunks = []
for x in range(0, len(RESIDS), 5):
    chunks.append(RESIDS[x : x + 5])
segments_V0 = []
for chunk in chunks:
    segment = []
    for res in chunk:
    ...
    if not segments_V1:
        ...
        if not isdone:
            if len(candidates) == 2:
                ...
                if inter_prob * 1.07 >= intra_prob1 or inter_prob * 1.07 >= intra_prob2:
                    new_segments = []
                    new_segment = set([])
                    for counts, segment in enumerate(segments_V1):
...
rp = open(f'{prefix}.finalDPAM.domains', 'w')
for countd, domain in enumerate(domains_v2):
    rp.write(f'D{countd + 1}\t{domain[1]}\n')
rp.close()
