...
fp = open(script_dir+'/ECOD_pdbmap','r')
pdb2ecod = {}
good_hids = set([])
for line in fp:
    words = line.split()
...
for line in fp:
    ...
    ecod2len[words[0]] = len(words[2])
fp.close()


rp = open(prot + '.hhsearch_map_to_ecod.result', 'w')
rp.write('uid\tecod_domain_id\thh_prob\thh_eval\thh_score\taligned_cols\tidents\tsimilarities\tsum_probs\tcoverage\tungapped_coverage\tquery_range\ttemplate_range\ttemplate_seqid_range\n')
for hit in allhits:
    hid = hit[0]
    pdbid = hid.split('_')[0]
        ...
        for ecod in ecods:
            ...
            if len(qseq) == len(hseq):
                ...
                if len(qresids) >= 10 and len(eresids) >= 10:
                    ...
                    rp.write(ecod + '\t' + ecodkey + '\t' + hh_prob + '\t' + hh_eval + '\t' + hh_score + '\t' + aligned_cols + '\t' + idents + '\t' + similarities + '\t' + sum_probs + '\t' + str(coverage) + '\t' + str(ungapped_coverage) + '\t' + qrange + '\t' + erange + '\t' + hrange + '\n')
            else:
