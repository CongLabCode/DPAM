...
if not os.path.exists('step23'):
    os.system('mkdir step23/')
if not os.path.exists('step23/' + dataset):
    os.system('mkdir step23/' + dataset)
...
for prot in prots:
    if os.path.exists('step23/' + dataset + '/' + prot + '.assign'):
        ...
        fp.close()
        if len(word_counts) == 1 and 10 in word_counts:
            pass
        else:
            os.system('rm step23/' + dataset + '/' + prot + '.assign')
            need_prots.append(prot)
    else:
        if os.path.exists('step23/' + dataset + '/' + prot + '.done'):
            pass
        else:
            need_prots.append(prot)
...
if need_prots:
    ...
    for prot in need_prots:
        cmds.append('step23_get_predictions.py ' + dataset + ' ' + prot + '\n')
    logs = batch_run(cmds, ncore)
    fail = [i for i in logs if 'fail' in i]
    if fail:
        with open(dataset + '_step23.log','w') as f:
            for i in fail:
                f.write(i+'\n')
    else:
        with open(dataset + '_step23.log','w') as f:
