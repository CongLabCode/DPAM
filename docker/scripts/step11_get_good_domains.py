...
fp = open('/mnt/databases/ECOD_norms', 'r')
ecod2norm = {}
for line in fp:
    words = line.split()
...
prot= sys.argv[2]
results = []
if os.path.exists(f'step10/{spname}/{prot}_sequence.result'):
    fp = open(f'step10/{spname}/{prot}_sequence.result', 'r')
    for line in fp:
        words = line.split()
        filt_segs = []
...
if os.path.exists(f'step10/{spname}/{prot}_structure.result'):
    fp = open(f'step10/{spname}/{prot}_structure.result', 'r')
    for line in fp:
        words = line.split()
...
if results:
    rp = open(f'step11/{spname}/{prot}.goodDomains', 'w')
    for line in results:
        rp.write(line)
    rp.close()
else:
    os.system(f'echo \'done\' > step11/{spname}/{prot}.done')
