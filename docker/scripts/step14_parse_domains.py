...
import numpy as np
import os, sys, json, math, string, subprocess
...
diso_resids = set([])
fp = open(f'step13/{dataset}/{prot}.diso', 'r')
for line in fp:
    words = line.split()
...
fp = open(f'step2/{dataset}/{prot}.pdb','r')
resid2coors = {}
resids = []
for line in fp:
    if len(line) >= 50:
    ...
    for res2 in RESIDS:
        if res1 < res2:
            ...
            if check_dist and check_error and check_hhpro and check_daliz:
                ...
                print (f'error\t{prot}\t{str(res1)}\t{str(res2)}')
...
if domains_v2:
    rp = open(f'step14/{dataset}/{prot}.domains', 'w')
    for countd, domain in enumerate(domains_v2):
        rp.write(f'D{str(countd + 1)}\t{domain[1]}\n')
    rp.close()
else:
    os.system(f'echo \'done\' > step14/{dataset}/{prot}.done')
